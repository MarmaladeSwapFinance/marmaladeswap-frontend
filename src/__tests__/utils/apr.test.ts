...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { marmRewardsApr: cakeRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null)
    expect(cakeRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { marmRewardsApr: cakeRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '')
    expect(cakeRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { marmRewardsApr: cakeRewardsApr, lpRewardsApr } = getFarmApr(
      BIG_TEN,
      new BigNumber(1),
