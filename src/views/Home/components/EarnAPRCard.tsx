...
import { Heading, Card, CardBody, Flex, ArrowForwardIcon, Skeleton } from '@marmaladeswap/uikit'
...
import { useFarms, usePriceMarmBusd } from 'state/farms/hooks'
    ...
    if (marmPrice.gt(0)) {
      const aprs = farmsLP.map((farm) => {
        ...
        if (farm.pid !== 0 && farm.multiplier !== '0X' && farm.lpTotalInQuoteToken && farm.quoteToken.busdPrice) {
          ...
          const { marmRewardsApr, lpRewardsApr } = getFarmApr(
            new BigNumber(farm.poolWeight),
            marmPrice,
            totalLiquidity,
            farm.lpAddresses[ChainId.MAINNET],
          )
          return marmRewardsApr + lpRewardsApr
        }
        return null
      })

      console.log('maxapr', aprs)
